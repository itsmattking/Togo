  ...
  module ActiveRecord
    module Model
      ...
      module ClassMethods
        ...
        def stage_content(content,attrs)
          content.attributes = content_columns.inject({}){|m,p| attrs[p.name.to_sym] ? m.merge!(p.name.to_sym => attrs[p.name.to_sym]) : m}
          reflect_on_all_associations.each do |r| 
            val = attrs["related_#{r[0]}".to_sym]
            next if not val or val == 'unset'
            content = RelationshipManager.create(content, r, :ids => val).relate
          end
        ...
        def sorting_from_property(property)
          property.is_a?(::ActiveRecord::ConnectionAdapters::Column)
        end
        ...
        def shown_properties
#           skip = relationships.values.collect{|r| r.through if r.respond_to?(:through) }.compact.uniq # don't include join models
          content_columns.select{|p| not BLACKLIST.include?(p.name) and not p.name =~ /_id$/} + reflect_on_all_associations #.select{|r| not skip.include?(r)}
        end

        def search_properties
          only_properties = []
          content_columns.select{|p| only_properties.include?(p.type)} # type seems to be depracated in 1.0
        end
